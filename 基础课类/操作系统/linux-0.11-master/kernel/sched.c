...
#define _S(nr) (1 << ((nr)-1))
// 除了SIGKILL
// 和SIGSTOP信号以外其他信号都是可阻塞的(...1011,1111,1110,1111,111b)
...
void show_task(int nr, struct task_struct* p) {
    ...
    i = 0;
    while (i < j &&
           !((char*)(p + 1))[i])  // 检测指定任务数据结构以后等于0的字节数。
        i++;
...
// PC机8253定时芯片的输入时钟频率约为1.193180MHz.
// Linux内核希望定时器发出中断的频率是
...
struct task_struct* current =
    &(init_task.task);  // 当前任务指针(初始化指向任务0)
...
// 选择符(0x10),指针esp指在user_stack数组最后一项后面。这是因为Intel
// CPU执行堆栈操作 时是先递减堆栈指针sp值，然后在sp指针处保存入栈内容。
...
void schedule(void) {
    ...
    for (p = &LAST_TASK; p > &FIRST_TASK; --p)
        if (*p) {
            ...
            // 置任务为就绪状态。其中'~(_BLOCKABLE &
            // (*p)->blocked)'用于忽略被阻塞的信号，但 SIGKILL
            // 和SIGSTOP不能呗阻塞。
...
int ticks_to_floppy_on(unsigned int nr) {
    ...
    // 并且如果要求启动的马达还没有启动，则置相应软驱的马达启动定时器值(HZ/2 =
    // 0.5秒
...
static struct timer_list {
    ...
    *next_timer = NULL;  // next_timer是定时器队列头指针
...
// 到设置定时操作时系统滴答值jiffies和转换成滴答单位的定时值之和，即'jiffies +
// HZ*定时秒值'。
...
void sched_init(void) {
    ...
    // NT标志用于控制程序的递归调用(Nested
    // Task)。当NT置位时，那么当前中断任务执行
